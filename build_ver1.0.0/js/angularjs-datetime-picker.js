(function() {
  ...
  var DatetimePickerCtrl = function($compile, $document) {
    ...
    this.openDatetimePicker = function(options) {
      ...
      if (options.timeOnly === '' || options.timeOnly === true || options.timeOnly === 'time-only') {
  ...
  var tmpl = [
    ...
    '  <div class="adp-month" id="adp-top">',
    ...
    '  </div>',
    '  <div class="adp-days" id="adp-middle" ng-click="setDate($event)">',
  ...
  var datetimePickerPopup = function($locale, dateFilter){
    ...
    var linkFunc = function(scope, element, attrs, ctrl) { //jshint ignore:line
      ...
      if (scope.timeOnly === true){
        element[0].querySelector('#adp-top').style.display = 'none';
        element[0].querySelector('#adp-middle').style.display = 'none';
      }
